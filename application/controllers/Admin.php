...
class Admin extends CI_Controller{
    ...
    function dashboard() {
        ...
        $this->load->view('backend/index', $page_data);
         ...
         /****MANAGE PD*****/
    function pd($param1 = '', $param2 = '')
    {
        if ($param1 == 'create') {
            $data['name']         = $this->input->post('name');
            $this->db->insert('pd', $data);
            $this->session->set_flashdata('flash_message' , 'Input data sukses');
            redirect(base_url() . 'admin/pd/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']         = $this->input->post('name');
            
            $this->db->where('id', $param2);
            $this->db->update('pd', $data);
        ...
        $page_data['classe']    = $this->db->get('pd')->result_array();
    ...
    function ujian($param1 = '', $param2 = '')
    {
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('flash_message' , 'Input data sukses');
            redirect(base_url() . 'admin/ujian/', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']         = $this->input->post('name');
            $data['bulan']         = $this->input->post('bulan');
        ...
        $page_data['classe']    = $this->db->get('ujian')->result_array();
	...
	function student_add() {
		...
		$page_data['page_title'] = 'Tambah Data Santri';
		$this->load->view('backend/index', $page_data);
	}
	...
	function student_bulk_add($param1 = '')
	{
		...
		{
			move_uploaded_file($_FILES['userfile']['tmp_name'], 'uploads/import_santri.xlsx');
			// Importing excel sheet for bulk student uploads

			include 'Simplexlsx.class.php';
			
			$xlsx = new SimpleXLSX('uploads/import_santri.xlsx');
			...
			foreach( $xlsx->rows() as $r ) 
			{
				...
				for( $i=0; $i < $num_cols; $i++ )
				{
					if ($i == 0){
                        $data['nis']			=	$r[$i];
                        $data['email']      =   $r[$i]."@darulamal.sch.id";
                    }
					else if ($i == 1)	
                        {$data['name']		=	$r[$i];}
					else if ($i == 2)	{
		...
		$page_data['page_title'] = 'Import Data Santri';
		$this->load->view('backend/index', $page_data);
	}
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['tl']   = $this->input->post('tempat');
            $data['sex']        = $this->input->post('sex');
            $data['address']    = $this->input->post('address');
            $data['class_id']   = $this->input->post('class_id');
            $data['section_id'] = $this->input->post('section_id');
            $data['email'] = $this->input->post('nis')."@darulamal.sch.id";
            $data['password'] = md5($this->input->post('birthday'));
    ...
    function teacher($param1 = '', $param2 = '', $param3 = '')
    {
        if ($param1 == 'create') {
            ...
            $data['address']     = $this->input->post('address');
            $data['phone']       = $this->input->post('phone');
            $data['email']       = $this->input->post('nipy')."@darulamal.sch.id";
            $data['password']       = md5($this->input->post('nipy'));
        ...
        $page_data['page_title'] = 'Data Guru';
        $this->load->view('backend/index', $page_data);
    }
    ...
    function subject($param1 = '', $param2 = '' , $param3 = '')
    {
        if ($param1 == 'create') {
            ...
            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            redirect(base_url() . 'admin/subject/'.$data['section_id'], 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']       = $this->input->post('name');
            $data['section_id']   = $this->input->post('section_id');
    ...
    function classes($param1 = '', $param2 = '')
    {
        ...
        $page_data['classe']    = $this->db->get('class')->result_array();
        $page_data['page_name']  = 'class';
        $page_data['page_title'] = 'Jenjang Pendidikan';
        $this->load->view('backend/index', $page_data);
    }
...
/****** DAILY ATTENDANCE *****************/
    function manage_attendance($month='',$year='',$class_id='',$section_id='')
    {
    ...
    {
        redirect(base_url() . 'admin/manage_attendance/'.
      ...
      function akademik($month='',$year='',$class_id='',$section_id='')
    {
        if($this->session->userdata('admin_login')!=1)redirect('login' , 'refresh');
         $mapel = $this->db->get_where('subject',array('section_id'=>$section_id))->result_array();

        
        $page_data['mapel'] =  $mapel;
        $page_data['section_id'] =  $section_id;
        $page_data['month']    =    $month;
        $page_data['year']     =    $year;
        $page_data['class_id'] =    $class_id;
        ...
        $page_data['page_title'] =  'Laporan Nilai Akademik Santri';
        $this->load->view('backend/index', $page_data);
    }
    function akademik_selector()
    {
        redirect(base_url() . 'admin/akademik/'.
                    $this->input->post('bulan').'/'.
                        $this->input->post('year').'/'.
                            $this->input->post('class_id').'/'.
                             $this->input->post('section_id'), 'refresh');
         ...
         function hafalan($month='',$year='',$class_id='',$section_id='')
    {
        if($this->session->userdata('admin_login')!=1)redirect('login' , 'refresh');
        
         $mapel = $this->db->get_where('hafalan',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();

         $vocab = $this->db->get_where('vocab',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();

         $btq = $this->db->get_where('btq',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();

        $page_data['btq'] =  $btq;
        $page_data['vocab'] =  $vocab;
        $page_data['mapel'] =  $mapel;
        $page_data['section_id'] =  $section_id;
        $page_data['month']    =    $month;
        $page_data['year']     =    $year;
        ...
        $page_data['page_title'] =  'Tahfidz';
        $this->load->view('backend/index', $page_data);
    }
    function hafalan_selector()
    {
        redirect(base_url() . 'admin/hafalan/'.
                    $this->input->post('bulan').'/'.
                        $this->input->post('year').'/'.
                            $this->input->post('class_id').'/'.
                             $this->input->post('section_id'), 'refresh');
    }
    function kesantrian($month='',$year='',$class_id='',$section_id='')
    {
        if($this->session->userdata('admin_login')!=1)redirect('login' , 'refresh');
        
         $keg = $this->db->get_where('kegiatan',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();

         $pres = $this->db->get_where('prestasi',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();

         $izin= $this->db->get_where('izin',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();
          $sakit = $this->db->get_where('sakit',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();

         $pel = $this->db->get_where('pelanggaran',array('section_id'=>$section_id,
                                                        'bulan' => $month))->result_array();
    ...
    function kesantrian_selector()
    {
        redirect(base_url() . 'admin/kesantrian/'.
                    $this->input->post('bulan').'/'.
                        $this->input->post('year').'/'.
                            $this->input->post('class_id').'/'.
                             $this->input->post('section_id'), 'refresh');
