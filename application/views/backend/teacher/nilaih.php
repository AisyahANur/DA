<hr />
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="table table-bordered">
        ...
        <tbody>
            <form method="post" action="<?php echo base_url();?>teacher/nhafalan_selector" class="form">
                <tr class="gradeA">
                    ...
                    <td>
                        <select name="kategori" class="form-control" data-validate="required" id="class_id" data-message-required="<?php echo get_phrase('value_required');?>" onchange="return get_class_sections(this.value)">
                              ...
                              <option value="0">Hadits</option>
                        </select>

                    </td>

                    <td>
                        <select name="hafalan_id" class="form-control" data-validate="required" id="section_selector_holder" >
                              <option value="">Pilih</option>
...
<?php if( $month!='' && $year!='' && $section_id!='' && $hafalan_id !=0):
?>
        ...
        <div class="col-sm-offset-4 col-sm-4">
            ...
            <div class="tile-stats tile-purple">
                ...
                <?php
                    ...
                    $day        = date('M', $month);
                 ?>
                <h2><?php echo ucwords($this->crud_model->get_tema('hafalan','hafalan_id',$hafalan_id));?></h2>
                        ...
                        <tr class="gradeA">
                            ...
                            <?php 
                                ...
                                $verify_data    =   array(  'nis' => $row['nis'],
                                                            ...
                                                            'hafalan_id' => $hafalan_id);
                                $query = $this->db->get_where('nilai_hafalan' , $verify_data);
                                if($query->num_rows() < 1)
                                $this->db->insert('nilai_hafalan' , $verify_data);
                                
                                //showing the attendance status editing option
                                $nilai = $this->db->get_where('nilai_hafalan' , $verify_data)->row();
...
<form method="post" 
    action="<?php echo base_url();?>teacher/manage_nilaih/<?php echo $month.'/'.$year.'/'.$section_id.'/'.$hafalan_id;?>">
        <div class="col-sm-offset-3 col-md-6">
            <table  class="table table-bordered">
                <thead>
                    <tr class="gradeA">
...
<script >
    ...
    function get_class_sections(class_id) {
        ...
        $.ajax({
            url: '<?php echo base_url();?>teacher/get_tema_hafalan/' + class_id + '/<?= $section_id;?>/'+bulan,
